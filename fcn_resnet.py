...
class fcn(nn.Module):
    def __init__(self, class_num):
        super(fcn, self).__init__()
        self.block = nn.Sequential(*list(pretrain.children())[:-2])
...
net.load_state_dict(torch.load('fcn_resnet_crop_index_myself_33335555.pth'))
...
torch.save(net.state_dict(),'fcn_resnet_crop_index_myself_33334444.pth')
'''
...
# net.load_state_dict(torch.load('fcn_resnet_crop_index_myself_33334444.pth'))
net.load_state_dict(torch.load('fcn_resnet_crop_index_myself_33335555.pth'))
net.eval()
for i in range(t):
