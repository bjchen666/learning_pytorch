...
class fcn(nn.Module):
    def __init__(self,class_num):
        super(fcn, self).__init__()
        self.block1 = nn.Sequential(net_list[0:5])
        self.block2 = nn.Sequential(net_list[5:10])
        self.block3 = nn.Sequential(net_list[10:17])
        self.block4 = nn.Sequential(net_list[17:24])
        self.block5 = nn.Sequential(net_list[24:31])

        self.block6 = nn.Conv2d(512,class_num,1)

        self.block7 = nn.ConvTranspose2d(class_num, class_num, 64, 32, 16, bias=False)
        self.block7.weight.data = bilinear_kernel(class_num, class_num, 64)
