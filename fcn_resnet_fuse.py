...
with open('result_resnet.csv', 'a') as f:
    f.write(str + '\n')
...
class fcn(nn.Module):
    def __init__(self, class_num):
        super(fcn, self).__init__()
        self.block1 = nn.Sequential(*list(pretrain.children())[:-4])
        self.block2 = nn.Sequential(*list(pretrain.children())[-4])
        self.block3 = nn.Sequential(*list(pretrain.children())[-3])

        self.transfrom1 = nn.Conv2d(128, class_num, 1)
        self.transfrom2 = nn.Conv2d(256, class_num, 1)
...
torch.save(net.state_dict(),'fcn_resnet_fuse_crop_index_myself.pth')
# '''
...
# net.load_state_dict(torch.load('fcn_resnet_fuse_crop_index_myself_33334444.pth'))  ##old
# plt.suptitle('old')
# # net.load_state_dict(torch.load('fcn_resnet_fuse_crop_index_myself_33335555.pth')) ##new
# # plt.suptitle('new')
# net.eval()
# for i in range(t):
#     img, label = test_set[i+100]
#     title=test_set.lines[i+100]
